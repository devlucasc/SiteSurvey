...
public abstract class ImageTool {
	 ...
	 */
	public BufferedImage[] splitImage(Image img, int rows, int cols){
	 ...
	 */
	public Image createTiledImage(Image img, int width, int height){
		// Create a null image
		Image image = null;
		BufferedImage bimg = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
	 ...
	 * @return The created image
	 */
	public Image getColoredImage(Color color, int width, int height){
	 ...
	 * @return The image after flip
	 */
	public Image flipImageVertically(Image img){
		int w = img.getWidth(null);
		int h = img.getHeight(null);
		BufferedImage bimg = toBufferedImage(getEmptyImage(w, h));
		Graphics2D g = bimg.createGraphics();
	 ...
	 */
	public Image mask(Image img, Color color){
		BufferedImage bimg = toBufferedImage(getEmptyImage(img.getWidth(null), img.getHeight(null)));
		Graphics2D g = bimg.createGraphics();
		g.drawImage(img, 0, 0, null);
		g.dispose();
