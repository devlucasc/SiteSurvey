...
public class ProgressDialog extends JDialog implements ChangeListener{ 
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7273959605339261158L;
	...
	public void stateChanged(final ChangeEvent ce){
		...
		SwingUtilities.invokeLater(new Runnable() {     
			...
			public void run() {
				synchronized (this) {
					int minMon = 0;
					int maxMon = monitor.getTotal();
					int curMon = monitor.getCurrent();
					int percent = (curMon*100)/(maxMon - minMon);

					if(percent == 100) dispose();
					statusLabel.setText(String.format("Progress.. %d %%", percent));
					progressBar.setMinimum(0);
					progressBar.setMaximum(maxMon);
					progressBar.setValue(curMon);
				}
			}
		});
